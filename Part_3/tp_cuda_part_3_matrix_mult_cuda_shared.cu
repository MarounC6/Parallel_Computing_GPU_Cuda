/*
**  PROGRAM: Matrix Multiply - CUDA Shared Memory Version
**
...
**                C  = A * B
**
**  Q3.5: Improve kernel to support multiple threads per block with shared memory and atomics
**
...
// CUDA Kernel: Multiple threads per block with shared memory
// Each block computes one tile of C using shared memory
__global__
void matrixMultSharedKernel(double* A, double* B, double* C, int Ndim, int Pdim, int Mdim)
{
    // Thread indices
    int tx = threadIdx.x;
    int ty = threadIdx.y;
...
void matrixMultCuda(double* A, double* B, double* C, int Ndim, int Pdim, int Mdim)
{
    ...
    // Grid dimensions: (Mdim/TILE_SIZE) x (Ndim/TILE_SIZE) blocks
